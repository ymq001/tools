...
(function () {
    ...
    */
    ...
    LuShu.prototype.start = function (_end_callback) {
        var me = this,
            len = me._path.length;
        //2017.12.5 Add by ZhuZhan 添加结束回调
        if (this.end_callback) {
            return;  
        }

        if (_end_callback) {
            this._end_callback = _end_callback;
        }
        ...
        _troughPointIndex: function (markerPoi) {
            ...
            for (var i = 0, len = t.length; i < len; i++) {
                ...
                if (!t[i].bShow) {
                    ...
                    //两点距离小于10米，认为是同一个点
                    if (distance < 10) {
                        //t[i].bShow = true;    //2017.12.6 每次经过设置点的时候都显示
                        return i;
                    }
                }
            }
    ...
    CustomOverlay.prototype.initialize = function (map) {
        ...
        //2017.12.4 Modify By ZhuZhan 修改样式以适应需求
        var div = this._div = baidu.dom.create('div', { style: 'border:solid 1px #ccc;width:auto;min-width:60px;text-align:center;position:absolute;background:#fff;color:#000;font-size:12px;border-radius: 5px;white-space: nowrap;' });
