...
(function() {
      ...
      if (self.map) {
        ...
        for (var i = 0; i < self.cornerPoints.length; i++) {
          var marker = new BMap.Marker(self.cornerPoints[i], {
            icon: new BMap.Icon('../../../Scripts/map/images/circle.png', new BMap.Size(16,16)),
            enableDragging: true,
            raiseOnDrag: true
          });
  ...
  function getCurvePoints(points) {
    ...
    for (var i = 0; i < points.length - 1; i++){
      ...
      //var p = getCurveByTwoPoints(points[i], points[i + 1]);
        var p = getPointList(points[i], points[i + 1]);
        console.log(p);
      if (p && p.length > 0) {
  ...
  function getCurveByTwoPoints(obj1, obj2) {
    ...
    var count=1000; // 曲线是由一些小的线段组成的，这个表示这个曲线所有到的折线的个数
    var isFuture=false;
  ...
  function getPointList(obj1, obj2) {
      if (!obj1 || !obj2 || !(obj1 instanceof BMap.Point) || !(obj2 instanceof BMap.Point)) {
          return null;
      }
      var points = [[map.pointToPixel(obj1)], [map.pointToPixel(obj2)]];
      var ex = points[1][0];
      var ey = points[1][1];
      points[3] = [ex, ey];
  ...
  function getOffsetPoint(start, end) {
      var distance = getDistance(start, end) / 3; //除以3？
      var angle, dX, dY;
